...
func (_m *WorkerMock) NewTxTracker(tx pool.Transaction, counters state.ZKCounters, ip string) (*TxTracker, error) {
	...
	var r0 *TxTracker
	var r1 error
	if rf, ok := ret.Get(0).(func(pool.Transaction, state.ZKCounters, string) (*TxTracker, error)); ok {
		return rf(tx, counters, ip)
	}
	if rf, ok := ret.Get(0).(func(pool.Transaction, state.ZKCounters, string) *TxTracker); ok {
	...
	if rf, ok := ret.Get(1).(func(pool.Transaction, state.ZKCounters, string) error); ok {
