...
- A machine to run the xlayer node with the following requirements:
...
- A L1 node: we recommend using geth, but what it's actually needed is access to a JSON RPC interface for the L1 network (Goerli for xlayer testnet, Ethereum mainnet for xlayer mainnet)

## Setup

This is the most straightforward path to run a xlayer node, and it's perfectly fine for most use cases, however if you are interested in providing service to many users it's recommended to do some tweaking over the default configuration. Furthermore, this is quite opinionated, feel free to run this software in a different way, for instance it's not needed to use Docker, you could use the Go and C++ binaries directly.
...
curl -L https://github.com/okx/xlayer-node/releases/latest/download/$ZKEVM_NET.zip > $ZKEVM_NET.zip && unzip -o $ZKEVM_NET.zip -d $ZKEVM_DIR && rm $ZKEVM_NET.zip
...
- Run dedicated instances for Postgres. To achieve this you will need to:
  - Remove the Postgres services (`xlayer-pool-db` and `xlayer-state-db`) from the `docker-compose.yml`
