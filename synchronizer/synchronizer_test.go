...
func TestForcedBatch(t *testing.T) {
	...
	sync, err := NewSynchronizer(false, m.Etherman, ethermanForL1, m.State, m.Pool, m.EthTxManager, m.ZKEVMClient, nil, genesis, cfg, false, m.DataCommitteeClientFactory)
	require.NoError(t, err)
...
func TestSequenceForcedBatch(t *testing.T) {
	...
	m.Etherman.
		...
		Return(nil, nil)
	ethermanForL1 := []EthermanInterface{m.Etherman}
	sync, err := NewSynchronizer(true, m.Etherman, ethermanForL1, m.State, m.Pool, m.EthTxManager, m.ZKEVMClient, nil, genesis, cfg, false, m.DataCommitteeClientFactory)
	require.NoError(t, err)
